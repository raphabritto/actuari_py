...
PROC IML;
	use WORK.indexador_monetario;
		read all var { PC_REAJUSTE } into FtBenMin2_ where(ID_REFERENCIA_INDEXADOR = 1);
		read all var { PC_REAJUSTE } into FtInssAss_ where(ID_REFERENCIA_INDEXADOR = 2);
		read all var { PC_REAJUSTE } into FtInssCon_ where(ID_REFERENCIA_INDEXADOR = 3);
