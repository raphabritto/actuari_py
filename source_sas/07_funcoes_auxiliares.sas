...
PROC IML;
	start GetContribuicaoPercentual(salario, percentual, beneficioRisco, despesaAdmin, apx, isParticipante);
		contribuicaoPercentual = 0;

		if (&CdPlanBen = 4) then do;
			contribuicaoPercentual = Max(0, round((salario * percentual * (1 - despesaAdmin) - salario * beneficioRisco) * &NroBenAno * (1 - apx), 0.01));
		end;
		else if (&CdPlanBen = 5) then do;
			if (isParticipante = 1) then do;
				contribuicaoPercentual = Max(0, round(salario * percentual * (1 - despesaAdmin) * &NroBenAno * (1 - apx), 0.01));
			end;
			else do;
				contribuicaoPercentual = Max(0, round((salario * percentual * (1 - despesaAdmin) - salario * beneficioRisco) * &NroBenAno * (1 - apx), 0.01));
			end;
...
/*	START CalcSalarioInss(CdSexoPartic_, VlBenefiInss_, DtApoEntPrev, IddApoEntPre, TmpInssContr, SalBenefInss_, FtPrevideAtc);*/
	START CalcSalarioInss(CdSexoPartic_, DtApoEntPrev, IddApoEntPre, TmpInssContr, SalBenefInss_, FtPrevideAtc);
			...
			if (DtApoEntPrev <= &DtLeiInss8595) then do;
				...
				if (CdSexoPartic_ = 1) then do;
					If (IddApoEntPre + TmpInssContr) >= 86 then
						...
						SalarioProjetadoInss = Max(Max(Round(SalBenefInss_ * FtPrevideAtc, 0.01), 0), &SalMinimo);
				end;
				else do;
