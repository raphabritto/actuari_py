	...
	if (nObs > 0) then do;
		...
		DO a = 1 TO nObs;
			...
			*DespPmsVP = max(0, round(DespPmsFxo * (1 / (1 + taxa_juros_cal)), 0.01));
			 DespPmsVP = max(0, round(DespPmsFxo * (1 / ((1 + taxa_juros_cal)/(1 + &PrTxBenef))), 0.01));
...
data sisatu.encargos_assistido;
	merge sisatu.encargos_assistido partic.assistidos;
	by id_participante;
	if CdTipoBenefi = 1 then tipo_beneficio = 'Aposentadoria tempo de contribuição';
	else if CdTipoBenefi = 2 then tipo_beneficio = 'Aposentadoria especial';
	else if CdTipoBenefi = 3 then tipo_beneficio = 'Aposentadoria invalidez';
	else tipo_beneficio = 'Pensao';
	*retain NuMatrPartic id_participante EncargoApo EncargoFut EncargoPms CdTipoBenefi fl_migrado;
	keep id_participante NuMatrPartic EncargoApo EncargoFut EncargoPms CdTipoBenefi fl_migrado tipo_beneficio;
run;
...
proc sql;
	...
	select (case
		   ...
		   sum(t1.VlBenefiPrev) format=commax18.2 as ValorMedioBenef,
