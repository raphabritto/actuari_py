...
%macro calcCoberturaPiv;
		...
		PROC IML;
			...
			if (qtdObs > 0) then do;
				...
				DO a = 1 TO qtdObs;
					...
					else if ((&CdPlanBen = 5 & DtIniBenInss <= DtAdesaoPlan) | ((&CdPlanBen = 4 | &CdPlanBen = 5) & VlBenefiInss = 0)) then do;
	...
	proc delete data = work.piv_cobertura_ativos (gennum=all);
	run;
%mend;
%calcCoberturaPiv;


%macro calcula_encargo_piv_alm;
	%if (&CdPlanBen. = 4 | &CdPlanBen. = 5) %then %do;
		%_eg_conditional_dropds(work.encargo_piv_alm);
		proc sql;
			create table work.encargo_piv_alm as
			select c.id_participante, c.t, max(0, (c.BenLiqCobPIV * (1 - c.apx) * c.ix * (c.ajxcb - c.ajxx_i) * c.PrbCasado * &NroBenAno.)) format=commax14.2 as encargo_piv
			from cobertur.piv_ativos c
			order by c.id_participante, c.t;
		run;

		%_eg_conditional_dropds(cobertur.encargo_piv_alm);
		proc sql;
			create table cobertur.encargo_piv_alm as
			select e.t, max(0, sum(e.encargo_piv)) format=commax18.2 as encargo_piv
			from work.encargo_piv_alm e
			group by e.t
			order by e.t;
		run;

		proc delete data = work.encargo_piv_alm (gennum=all);
		run;
	%end;
%mend;
%calcula_encargo_piv_alm;
