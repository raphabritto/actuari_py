...
%macro calcula_cobertura_ptc;
	...
	proc delete data = work.ptc_cobertura_ativos (gennum=all);
	run;
%mend;
%calcula_cobertura_ptc;


%macro calcula_encargo_ptc_alm;
	%if (&CdPlanBen. = 4 | &CdPlanBen. = 5) %then %do;
		%_eg_conditional_dropds(work.encargo_ptc_alm);
		proc sql;
			create table work.encargo_ptc_alm as
			select c.id_participante, c.t, max(0, (c.BenLiqCobPTC * c.PrbCasado * (c.ajxcb - c.ajxx) * c.apx * &NroBenAno.)) format=commax14.2 as encargo_ptc
			from cobertur.ptc_ativos c
			order by c.id_participante, c.t;
		run;

		%_eg_conditional_dropds(cobertur.encargo_ptc_alm);
		proc sql;
			create table cobertur.encargo_ptc_alm as
			select e.t, max(0, sum(e.encargo_ptc)) format=commax18.2 as encargo_ptc
			from work.encargo_ptc_alm e
			group by e.t
			order by e.t;
		run;

		proc delete data = work.encargo_ptc_alm (gennum=all);
		run;
	%end;
%mend;
%calcula_encargo_ptc_alm;
