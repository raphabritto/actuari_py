...
%macro calcCoberturaAiv;
	...
	proc delete data = work.aiv_cobertura_ativos (gennum=all);
	run;
%mend;
%calcCoberturaAiv;


%macro calcula_encargo_aiv_alm;
	%if (&CdPlanBen. = 4 | &CdPlanBen. = 5) %then %do;
		%_eg_conditional_dropds(work.encargo_aiv_alm);
		proc sql;
			create table work.encargo_aiv_alm as
			select c.id_participante,
				   c.t,
				   max(0, (c.BenLiqCobAIV * (1 - c.apx) * c.ix * c.axiicb * &NroBenAno.)) format=commax14.2 as encargo_aiv
			from cobertur.aiv_ativos c
			order by c.id_participante, c.t;
		run;

		%_eg_conditional_dropds(cobertur.encargo_aiv_alm);
		proc sql;
			create table cobertur.encargo_aiv_alm as
			select e.t,
				   max(0, sum(e.encargo_aiv)) format=commax18.2 as encargo_aiv
			from work.encargo_aiv_alm e
			group by e.t
			order by e.t;
		run;

		proc delete data = work.encargo_aiv_alm (gennum=all);
		run;
	%end;
%mend;
%calcula_encargo_aiv_alm;
