...
%_eg_conditional_dropds(work.tabuas_temp, tabuas.tabuas);
PROC SQL;
	CREATE TABLE work.tabuas_temp AS
		...
		FROM sgca.TB_ATU_TABUA_BIOMETRICA t1, sgca.TB_ATU_TABUA_BIOMETRICA_VALOR t2, sgca.TB_ATU_AVALIACAO_TABUA t3
			WHERE (t1.ID_TABUA_BIOMETRICA = t2.ID_TABUA_BIOMETRICA AND t1.ID_TABUA_BIOMETRICA = t3.ID_TABUA_BIOMETRICA AND
				t3.ID_SEXO = t2.ID_SEXO) AND (t3.ID_AVALIACAO = &id_avaliacao)
			ORDER BY t1.ID_TIPO_TABUA,
				t2.ID_SEXO,
				t2.NR_IDADE;

	create table tabuas.tabuas as
		select a.id_avaliacao, tt.ds_tipo_tabua, t.nm_tipo_tabua,
		       	(case
		          when a.id_sexo = 1
		            then 'F'
		            else 'M'
		        end) as SEXO
		from sgca.tb_atu_avaliacao_tabua a
		inner join sgca.tb_atu_tabua_biometrica t on (a.id_tabua_biometrica = t.id_tabua_biometrica)
		inner join sgca.tb_atu_tipo_tabua tt on (t.id_tipo_tabua = tt.id_tipo_tabua)
		where a.id_avaliacao = &id_avaliacao
		order by tt.ds_tipo_tabua, a.id_sexo;
QUIT;
